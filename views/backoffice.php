            ...
            <div class="posts-grid">
                ...
                <div class="post-card" data-post-id="<?= $post['id'] ?>">
                    ...
                    <div class="post-content">
                        <div class="post-text"><?= nl2br(htmlspecialchars(substr($post['content'],0,200))) ?><?= strlen($post['content']) > 200 ? '…' : '' ?></div>
                        <button class="listen-post-btn" onclick="speakPost(<?= $post['id'] ?>, this.previousElementSibling)"><i class="fas fa-headphones"></i> <?= $t('listen') ?></button>
                    </div>
                    <div class="post-meta">
...
<script>
    ...
    new Chart(document.getElementById('barChart'), {
        type: 'bar',
        data: { labels: postsByDay.map(i=>i.date), datasets: [{ label: 'Publications', data: postsByDay.map(i=>i.count), backgroundColor: '#10b981', borderRadius: 8, barPercentage: 0.6 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top' } } }
    });
    new Chart(document.getElementById('pieChart'), {
        type: 'pie',
        data: { labels: ['<?= $t('with_image') ?>','<?= $t('with_video') ?>','<?= $t('text_only') ?>'], datasets: [{ data: [contentDist.with_image, contentDist.with_video, contentDist.text_only], backgroundColor: ['#10b981','#f59e0b','#8b5cf6'] }] },
        options: { responsive: true, maintainAspectRatio: false }
    });
    new Chart(document.getElementById('lineChart'), {
        type: 'line',
        data: { labels: activityTimeline.map(i=>i.date), datasets: [{ label: 'Posts créés', data: activityTimeline.map(i=>i.posts_count), borderColor: '#10b981', tension: 0.3, fill: true, backgroundColor: 'rgba(16,185,129,0.05)' }] },
        options: { responsive: true, maintainAspectRatio: false }
    });
